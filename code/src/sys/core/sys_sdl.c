...
#include "sys_local.h"
#include "../resources/sys_resource_sdl.h"
#include <SDL3/SDL.h>
...
//	vsprintf( text, error, argptr );
	...
	IN_Shutdown();
	Sys_DestroyConsole();
	SDL_ShowSimpleMessageBox( SDL_MESSAGEBOX_ERROR, "RtCW SDL3 Error", text, NULL );
	SDL_Quit();
...
int main( int argc, char **argv ) {
	...
	for ( i = 1; i < argc; i++ ) {
		...
		if ( quote ) {
			Q_strcat( sys_cmdline, sizeof( sys_cmdline ), "\"" );
		}
		Q_strcat( sys_cmdline, sizeof( sys_cmdline ), argv[i] );
		if ( quote ) {
			Q_strcat( sys_cmdline, sizeof( sys_cmdline ), "\"" );
		}
		Q_strcat( sys_cmdline, sizeof( sys_cmdline ), " " );
